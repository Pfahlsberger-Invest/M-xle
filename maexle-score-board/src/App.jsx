...
import { fetchRemoteGameState, loadGameState, saveGameState } from './scoreStore';
...
const DEFAULT_PEOPLE = [
  { id: 'p1', name: 'Luki', color: '#FF6B6B', inGame: true },
  { id: 'p2', name: 'Thoma', color: '#4ECDC4', inGame: true },
  { id: 'p3', name: 'Simi', color: '#FFD93D', inGame: true },
  { id: 'p4', name: 'Mauchi', color: '#A78BFA', inGame: true },
];
...
export default function App() {
  ...
  const applyPersistedState = (data) => {
    ...
    const storedScores = data.schandeScores || {};
    const storedLastDecay = data.lastDecay || Date.now();
    const now = Date.now();
    const elapsed = now - storedLastDecay;
    ...
    if (intervals > 0) {
      ...
      Object.keys(storedScores).forEach((id) => {
        decayed[id] = Math.max(SCORE_MIN, storedScores[id] - totalDecay);
      });
      setSchandeScores(decayed);
      setLastDecay(storedLastDecay + intervals * DECAY_INTERVAL_MS);
    } else {
      setSchandeScores(storedScores);
      setLastDecay(storedLastDecay);
    }
  };
  ...
  // Save globally whenever data changes
  useEffect(() => {
    if (!loaded) return;
    saveGameState({ people, clicks, schandeLog, schandeScores, lastDecay }).catch((e) => {
      console.error('Storage error:', e);
    });
  }, [people, clicks, schandeLog, schandeScores, lastDecay, loaded]);
  ...
  useEffect(() => {
    ...
    const tick = setInterval(async () => {
      try {
        const data = await fetchRemoteGameState();
        applyPersistedState(data);
      } catch {
        // Offline/local development fallback: keep the current in-memory state.
      }
    }, 10000);
    return () => clearInterval(tick);
  }, [loaded]);

  // Decay timer: every 5 minutes, scores drop by 10 (floor at -100)
  useEffect(() => {
    const tick = setInterval(() => {
      setSchandeScores((prev) => {
        const next = {};
        let changed = false;
        Object.keys(prev).forEach((id) => {
          const current = prev[id];
          if (current > SCORE_MIN) {
            next[id] = Math.max(SCORE_MIN, current - DECAY_AMOUNT);
            changed = true;
          } else {
            next[id] = current;
          }
  ...
  const togglePersonInGame = (id) => {
    setPeople((prev) =>
      prev.map((p) => (p.id === id ? { ...p, inGame: !p.inGame } : p))
    );
  };
  ...
  const handleRemove = (id) => {
    ...
    setClicks(clicks.filter((c) => c.personId !== id));
    setSchandeScores((prev) => {
      const next = { ...prev };
      delete next[id];
      return next;
  ...
  const applySchande = () => {
    ...
    setSchandeLog((prev) => [
      ...prev,
      { personId, delta, timestamp: Date.now(), ip: userIp || 'unknown' },
    ]);
    ...
    >
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-6 text-center mono text-[10px] text-stone-400">
          daten werden global gespeichert · browser-übergreifend
        </div>
      </div>
